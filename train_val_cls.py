...
def main():
    ...
    if setting.keep_remainder:
        ...
        batch_num_val = math.ceil(num_val / batch_size) # batch_num_val = count batch
    else:
    ...
    iterator = tf.data.Iterator.from_string_handle(handle, dataset_train.output_types) # feedable iterator,
    (pts_fts, labels) = iterator.get_next()

    pts_fts_sampled = tf.gather_nd(pts_fts, indices=indices, name='pts_fts_sampled') # gather_nd is differentiable
    ...
    points_augmented = pf.augment(points_sampled, xforms, jitter_range) # input含feature吗；要不要用这些Feature, rotate/jitter/scale加强
    ...
    with tf.Session() as sess:
        ...
        for batch_idx_train in range(batch_num):
            ...
            if (batch_idx_train % step_val == 0 and (batch_idx_train != 0 or args.load_ckpt is not None)) \
                ...
                for batch_idx_val in range(batch_num_val): # batch_idx_val = record which batch
                    ...
                    xforms_np, rotations_np = pf.get_xforms(batch_size_val, # xforms rely on batch_size_val
